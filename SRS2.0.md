...
|---|---|---|
| `eila_admin` | ผู้ดูแลระบบส่วนกลาง | จัดการทุกอย่างทั้งมหาวิทยาลัย |
| `faculty_admin` | ผู้ดูแลระดับคณะหรือหน่วยงาน | จัดการเว็บไซต์ ฟอร์ม และรายงานของ faculty ตัวเอง |
...
| FR-AUTH-20 | ระบบ SHALL รองรับ role override ด้วย Email OTP สำหรับ eila_admin เท่านั้น | Must Have | Both | OTP ถูกต้องแล้ว role เปลี่ยนสำเร็จและมี audit log |
...
| FR-WEB-02 | eila_admin SHALL จัดการ WebsiteTarget ได้ทั้งหมดแบบ CRUD | Must Have | Both | eila_admin สร้าง แก้ไข ลบ soft delete และดูทุกเว็บได้ |
| FR-WEB-03 | faculty_admin SHALL จัดการได้เฉพาะ WebsiteTarget ที่ `ownerFacultyId` ตรงกับ faculty ตัวเอง | Must Have | Both | faculty_admin แก้ไขเว็บข้าม faculty ไม่ได้ |
...
| FR-WEB-07 | ระบบ SHALL รองรับ bulk import WebsiteTarget จาก XLSX สำหรับ eila_admin | Should Have | Both | import XLSX ถูกต้องแล้วสร้างหลาย website target ได้ |
...
| FR-WEB-09 | ระบบ SHALL แจ้งเตือน eila_admin เมื่อพบ WebsiteTarget ที่ `urlStatus = unreachable` | Must Have | Backend | URL unreachable แล้วมี notification ถึง eila_admin |
...
| FR-ROUND-02 | eila_admin SHALL สร้าง round ระดับมหาวิทยาลัยได้ | Must Have | Both | round ที่ `scope = university` ถูกสร้างโดย eila_admin ได้ |
| FR-ROUND-03 | faculty_admin SHALL สร้าง round ระดับ faculty ของตัวเองได้ | Must Have | Both | faculty_admin สร้าง round faculty อื่นไม่ได้ |
...
| FR-FORM-10 | faculty_admin SHALL สร้างได้เฉพาะ `faculty` scope | Must Have | Both | faculty_admin เลือก university scope ไม่ได้ |
| FR-FORM-11 | eila_admin SHALL สร้างได้ทั้ง `faculty` และ `university` scope | Must Have | Both | eila_admin สร้างสอง scope ได้ |
...
| FR-TMPL-02 | eila_admin SHALL จัดการ template ทุกตัวได้ | Must Have | Both | eila_admin แก้ template ทุก faculty ได้ |
| FR-TMPL-03 | faculty_admin SHALL จัดการเฉพาะ template ของ faculty ตัวเอง | Must Have | Both | faculty_admin แก้ template ข้าม faculty ไม่ได้ |
...
| FR-TMPL-07 | เมื่อ faculty_admin owner ถูก soft delete ระบบ SHALL โอน ownership ของ template ไป eila_admin | Must Have | Backend | owner เดิมหายแล้ว template ยังมี owner ใหม่ |
| FR-TMPL-08 | eila_admin SHALL clone template ข้าม faculty ได้ | Should Have | Both | eila_admin clone template ไปอีก faculty ได้ |
| FR-TMPL-09 | faculty_admin SHALL clone ได้เฉพาะ template ของตัวเองภายใน faculty เดียวกัน | Should Have | Both | faculty_admin clone ข้าม faculty ไม่ได้ |
...
| ID | Requirement | Priority | Assigned To | Acceptance Criteria |
|---|---|---|---|---|
| FR-USER-01 | eila_admin SHALL จัดการผู้ใช้ role `faculty_admin` และ `executive` ได้ | Must Have | Both | create update delete user ได้เฉพาะ role ที่กำหนด |
...
| FR-USER-03 | ระบบ SHALL รองรับ bulk import user จาก XLSX สำหรับ eila_admin | Must Have | Both | import XLSX สำเร็จแล้วสร้างหรือ update user ได้หลายราย |
...
| FR-NOTIF-08 | หลัง retry ครบแล้วระบบ SHALL mark email เป็น `failed` และแจ้ง eila_admin | Must Have | Backend | failed 3 ครั้งแล้วมี admin alert |
...
| FR-NOTIF-10 | eila_admin SHALL ดู delivery status ได้จาก NotificationLog หรือ admin panel | Should Have | Both | admin เปิดหน้าดู log delivery ได้ |
...
| FR-DASH-04 | dashboard SHALL แสดง ranking เว็บไซต์ภายใน faculty หรือมหาวิทยาลัยตามสิทธิ์ผู้ใช้ | Must Have | Both | executive เห็นทั้งมหาวิทยาลัย faculty_admin เห็นเฉพาะ faculty ตัวเอง |
...
| FR-RANK-05 | eila_admin และ executive SHALL เห็น cross-faculty ranking ได้ | Must Have | Backend | executive เรียก ranking ทุก faculty ได้ |
| FR-RANK-06 | faculty_admin SHALL เห็น ranking เฉพาะ faculty ของตัวเอง | Must Have | Backend | faculty_admin query ข้าม faculty แล้วถูกจำกัด |
...
| FR-IE-08 | ระบบ SHALL export PDF รายงานคุณภาพเว็บไซต์รายเว็บได้สำหรับ eila_admin และ faculty_admin | Must Have | Backend | export PDF รายเว็บแล้วไฟล์ถูกสร้างสำเร็จ |
...
| FR-AUDIT-04 | eila_admin SHALL ดู audit log ทั้งระบบได้ | Must Have | Both | eila_admin เปิด audit log page ได้ |
...
| FR-DATA-07 | เมื่อ user ขอใช้สิทธิ์ลบข้อมูลตาม PDPA ระบบ SHALL มี workflow review approve reject โดย eila_admin | Must Have | Both | ส่งคำขอแล้วมีสถานะ workflow และผลลัพธ์ |
...
| NFR-SEC-04 | ระบบ SHALL ใช้ parameterized query ผ่าน Prisma และห้าม raw query ที่ไม่ sanitize | Must Have | Backend | code review ไม่พบ unsafe raw query |
...
| NFR-PERF-10 | ระบบ SHOULD monitor p95, p99, queue length, DB pool usage และ scheduler lag | Should Have | DevOps | monitoring dashboard แสดง metric ครบ |
...
| NFR-MAINT-04 | ระบบ SHALL มี migration history และ Prisma schema ที่ตรวจสอบย้อนกลับได้ | Must Have | Backend | migration สามารถ apply บน environment ใหม่ได้ |
...
- Backend SHALL ใช้ Fastify, Node.js, TypeScript, Prisma ORM
...
- cross-faculty ranking เป็นสิทธิ์ของ eila_admin และ executive เท่านั้น
...
### A. Database Schema (Prisma) — Logical Summary
...
| Actor | Use Cases |
|---|---|
| eila_admin | login, manage websites, manage users, manage rounds, manage templates, create forms, publish forms, view university dashboard, export reports, manage audit logs |
| faculty_admin | login, manage own websites, create faculty rounds, create faculty forms, assign evaluators, view faculty dashboard, export faculty website reports |
...
- Conditional Logic สำหรับแสดงหรือซ่อนคำถามตามคำตอบก่อนหน้า
- Auto Screenshot สำหรับ capture หน้าเว็บไซต์อัตโนมัติ
- AI Summary สำหรับสรุป long text feedback
- Public API สำหรับ integration ภายใน PSU
- Multi-language ภาษาไทยและอังกฤษ
- Advanced Analytics เช่น sentiment analysis และ benchmarking
