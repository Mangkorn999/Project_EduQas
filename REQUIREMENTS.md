...
> Version 1.4 | Updated: 2026-04-23
...
closed → open   ✅ (เปิดรอบใหม่ หรือขยายเวลา)ถ้าฟอร์ม closed → Respondent เห็นข้อความ "หมดเวลาการประเมินแล้ว"
---
...
- **FR-T04** การแชร์ Template ระหว่าง Faculty ต้องทำผ่าน **Export JSON → Import** เท่านั้น
...
Branchวัตถุประสงค์ใครใช้Merge ไปที่mainProduction เท่านั้นทั้งคู่ (ตกลงกันก่อน)—developรวมงานล่าสุดก่อน Releaseทั้งคู่mainfeature/xxxงานแต่ละชิ้นแต่ละคนdevelophotfix/xxxแก้ Bug เร่งด่วนบน Productionทั้งคู่main และ develop
❌ ห้าม push ตรงเข้า main เด็ดขาด
11.2 Hotfix Branch Strategyใช้เมื่อ: พบ Bug บน Production (main) ที่ต้องแก้ทันที ไม่สามารถรอ develop ได้# 1. แตก hotfix จาก main โดยตรง
git checkout main
...
git tag -a v1.4.1 -m "hotfix: form status bug"
...
11.3 Feature Branches ตามแผนงานคนที่ 1 — Backendfeature/backend-setup
...
11.4 Flow การทำงานประจำวัน# 1. ดึง develop ล่าสุดก่อนเริ่มงาน
git checkout develop
...
11.5 Commit Message ConventionPrefixใช้เมื่อตัวอย่างfeat:เพิ่มฟีเจอร์ใหม่feat: add PSU Passport OAuth flowfix:แก้ Bugfix: form status not updating on close_athotfix:แก้ Bug เร่งด่วนบน Productionhotfix: resolve duplicate response submissionchore:Config / Setup / Dependencieschore: setup Fastify with PostgreSQLrefactor:ปรับโค้ด ไม่เพิ่มฟีเจอร์refactor: extract form validation to middlewaredocs:แก้ไข Documentationdocs: update SRS v1.4style:แก้ไข UI / CSS เท่านั้นstyle: adjust form builder layout11.6 Pull Request Rules
...
eila — Web Evaluation System | SRS v1.4 | 2026-04-23
Prince of Songkla University