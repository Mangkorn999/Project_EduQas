...
export class ResponsesController {
  ...
  logWebsiteOpen = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    await this.service.logWebsiteOpen(formId, user.userId)
    return { ok: true }
  }
  ...
  getResponses = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await this.service.getResponses(formId, page, limit)
    return result
  }
  ...
  upsertResponse = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const data = await this.service.upsertResponse(formId, user.userId, answers)
  ...
  getResponse = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const data = await this.service.getResponse(responseId, user.userId, user.role)
      return { data }
    } catch (err: any) {
      const status = err.statusCode ?? 500
      return reply.code(status).send({ error: { code: err.code ?? 'error', message: err.message } })
  ...
  updateResponse = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const data = await this.service.updateResponse(responseId, user.userId, answers)
